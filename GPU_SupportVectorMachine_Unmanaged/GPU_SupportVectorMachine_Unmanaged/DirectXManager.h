...
namespace SVM_Framework{
	...
	public:
		...
		void setComputeShaderConstantBuffers(int devId, std::map<int,ID3D11Buffer*> &buffers);
		void setComputeShaderResourceViews(int devId, std::map<int,ID3D11ShaderResourceView*> &views);
		void setComputeShaderUnorderedAccessViews(int devId, std::map<int,ID3D11UnorderedAccessView*> &views);

		int getNumDevices() { return m_adapters.size(); }

		// Helpers
		template <class T>
		void copyFromGraphicsCard(int devId, ID3D11Buffer* buffer, std::vector<T> &copy){
			...
			m_adapters[devId].m_context->CopyResource(cpuBuffer,buffer);
		...
		template <class T>
		void copyFromAppendBuffer(int devId, ID3D11Buffer* buffer, ID3D11UnorderedAccessView* uav, std::vector<T> &copy){
			unsigned int size = 0;
			D3D11_MAPPED_SUBRESOURCE mappedResource;
			ID3D11Buffer *cpuBuffer;
			D3D11_BUFFER_DESC desc;
			...
			m_adapters[devId].m_deviceHandle->CreateBuffer(&desc,0,&cpuBuffer);
			m_adapters[devId].m_context->CopyStructureCount(cpuBuffer,0,uav);
		...
		template <class T>
		void copyToGraphicsCard(int devId, ID3D11Buffer* pBuffer, std::vector<T> &data){
			D3D11_MAPPED_SUBRESOURCE mappedResource;
			m_adapters[devId].m_context->Map(pBuffer,0,D3D11_MAP_WRITE_DISCARD,0,&mappedResource);

			std::copy(data.begin(),data.end(),(T*)mappedResource.pData);
			
			m_adapters[devId].m_context->Unmap(pBuffer,0);
		}

		template <class T>
		HRESULT createAppendConsumeBuffer(int devId, DWORD usage, DWORD access, UINT iNumElements, ID3D11Buffer** ppBuffer, ID3D11UnorderedAccessView** ppUAV, const T* pInitialData){
			HRESULT hr = S_OK;
		...
		template <class T>
		HRESULT createStructuredBufferUAV(int devId, DWORD usage, DWORD access, UINT iNumElements, ID3D11Buffer** ppBuffer, ID3D11UnorderedAccessView** ppUAV, const T* pInitialData){
			HRESULT hr = S_OK;
		...
		template <class T>
		HRESULT createStructuredBufferSRV(int devId, DWORD usage, DWORD access, UINT iNumElements, ID3D11Buffer** ppBuffer, ID3D11ShaderResourceView** ppUAV, const T* pInitialData){
			HRESULT hr = S_OK;
			...
			hr = m_adapters[devId].m_deviceHandle->CreateShaderResourceView( *ppBuffer, &srvDesc, ppUAV );
			if(FAILED(hr))
				return hr;
		...
		template <class T>
		HRESULT createConstantBuffer(int devId, ID3D11Buffer** ppBuffer, const T* pInitialData){
			HRESULT hr = S_OK;
